...
export default function Home() {
  ...
  // 1. ROZDĚLENÍ DAT (Základní kámen opravy)
  // linksOnly obsahuje vše kromě článků
  const linksOnly = useMemo(() => 
    allData.filter(item => item.category !== 'Článek'), 
  [allData]);

  // articlesOnly obsahuje pouze články
  const articlesOnly = useMemo(() => 
    allData.filter(item => item.category === 'Článek'), 
  [allData]);

  // 2. FILTRACE PRO MŘÍŽKU ODKAZŮ
  const filteredLinks = useMemo(() => {
    // Pokud jsme v kategorii Články, mřížku odkazů úplně vyprázdníme
    if (selectedCategory === 'Článek') return [];
    
    // Pokud jsme na "Vše", ukážeme jen bonusy (linksOnly)
    if (selectedCategory === 'all') return linksOnly;
    
    // Jinak klasické filtrování kategorií pro bonusy
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-100">
      ...
      <div className="relative max-w-6xl mx-auto px-4 py-12 sm:py-16">
        ...
        {/* MŘÍŽKA ODKAZŮ (Zobrazuje se jen pokud nejsme v kategorii Článek) */}
          ...
          {selectedCategory !== 'Článek' && (
            ...
            >
              {isLoading ? (
                ...
                <div className="text-center py-10 text-slate-500">Žádné bonusy nenalezeny.</div>
              )}
            </motion.div>
          )}
        </AnimatePresence>

        {/* SEKCE ČLÁNKY (Zobrazuje se POUZE při vybrané kategorii Článek) */}
        <AnimatePresence mode="wait">
          {selectedCategory === 'Článek' && (
            <motion.div 
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  {articlesOnly.map((article) => (
                    <motion.div 
                      key={article.id}
                      whileHover={{ y: -5 }}
                      className="group bg-white p-8 rounded-3xl border shadow-sm hover:shadow-xl transition-all"
                    >
                      ...
                      <div className="flex items-center text-sm font-bold text-slate-900">
                        Číst více <ArrowRight className="w-4 h-4 ml-1 group-hover:translate-x-2 transition-all" />
                      </div>
                    </motion.div>
                  ))}
                </div>
