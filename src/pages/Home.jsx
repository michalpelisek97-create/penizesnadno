...
export default function Home() {
  ...
  // 1. Rozdělení dat - TOTO JE KLÍČOVÉ
  ...
  // 2. Logika filtrace pro mřížku odkazů
  const filteredLinks = useMemo(() => {
    // Pokud jsme na záložce "Články", neukazujeme žádné referral karty
    if (selectedCategory === 'Článek') return [];
    
    // Pokud jsme na "Vše", ukážeme jen bonusy (ne články)
    if (selectedCategory === 'all') return linksOnly;
    
    // Jinak filtrujeme podle kategorie (crypto, banks, atd.)
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-100">
      <div className="relative max-w-6xl mx-auto px-4 py-12 sm:py-16">
        ...
        {/* MŘÍŽKA ODKAZŮ (Bonusy) - Skryje se, když klikneš na Články */}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  {articlesOnly.map((article) => (
                    <div key={article.id} className="bg-white p-8 rounded-3xl border border-slate-200/60 shadow-sm hover:shadow-md transition-all">
                      <div className="text-xs font-bold text-purple-600 uppercase mb-2">Příručka</div>
                      ...
                      <div className="flex items-center text-slate-900 font-bold group cursor-pointer">
                        Číst více <ArrowRight className="w-4 h-4 ml-2 group-hover:translate-x-1 transition-transform" />
                      </div>
                    </div>
                  ))}
                </div>
