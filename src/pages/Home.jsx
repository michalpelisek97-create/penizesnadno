...
// 1. Komponenta pro počítadlo (ponecháno beze změny)
...
export default function Home() {
  ...
  // 2. Google Verifikace & AdSense (vloženo pro Google Search Console)
  useEffect(() => {
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      else { await navigator.clipboard.writeText(window.location.href); alert('Odkaz byl zkopírován!'); }
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4 py-12 sm:py-16">
        ...
        <AnimatePresence mode="wait">
          {selectedCategory !== 'Článek' && (
            <motion.div key="links-section" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-20">
        ...
        {/* ČLÁNKY - OBNOVENO PŮVODNÍ ROZLOŽENÍ */}
        <AnimatePresence mode="wait">
          {(selectedCategory === 'all' || selectedCategory === 'Článek') && articles.length > 0 && (
            <motion.div key="articles-section" initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 40 }} className="mt-20">
